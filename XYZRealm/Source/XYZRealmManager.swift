...
public class XYZRealmManager {
    ...
    static var shared = XYZRealmManager()
    
    // 当前选择的账本名称，默认为 "default2"
    public var currentLedgerName: String = "default"
    
    // 配置账本的 schema 版本
    func updateRealmDatabase(for ledgerName: String? = nil) {
    ...
    // 获取特定账本文件路径
    private func getRealmFileURL(for ledgerName: String) -> URL {
    ...
    func selectLedger(_ ledgerName: String) {
        updateRealmDatabase(for: ledgerName)
    }
    
    // 展示分享 Realm 数据的功能
    func presentRealmDataShareVC(vc: UIViewController) {
